...
for bam_file in os.scandir(bam_dir):
    if fnmatch.fnmatch(bam_file.path, '*_Input_*.bam'):
        ...
        name = 'SPP_' + exp_num
        subprocess.run(f"Rscript /home/samzhao/F_seq/ChIP_seq_benchmark/TF_ChIP_Data/SPI1/phantompeakqualtools/run_spp.R -c={signal_file} -p=10 -fdr=1.0 -odir=./SPP_output_wo_control/ -savn=./SPP_output_wo_control/{name}.narrowpeak -savr=./SPP_output_wo_control/{name}.regionpeak", shell=True)
