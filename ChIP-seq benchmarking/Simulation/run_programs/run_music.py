...
for bam_file in os.scandir(bam_dir):
    if fnmatch.fnmatch(bam_file.path, '*_Input_*.bam'):
        ...
        subprocess.run(f"MUSIC -get_TF_peaks -begin_l 100 -end_l 200 -l_p 500 -step 1.5 -gamma 4 -chip ./MUSIC_output/{name}/Cpruned -control ./MUSIC_output/{name}/Ipruned -q_val 0.99 -l_mapp 50 -mapp /home/samzhao/F_seq/ChIP_seq_benchmark/Simulated_Data/random_genome", shell=True)
        subprocess.run(f"mv ERs_100.0_200.0_1.50_500_4.0.bed ./MUSIC_output/peaks/{name}.peaks", shell=True)
